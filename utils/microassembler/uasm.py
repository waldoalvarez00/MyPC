...
TEMPLATE = os.path.join(HERE, 'microcode', 'Microcode.sv.templ')
MIF_TEMPLATE = os.path.join(HERE, 'microcode', 'microcode.mif.templ')
VERILOG_ENUM_TEMPLATE = os.path.join(HERE, 'microcode', 'MicrocodeTypes.sv.templ')
CPP_ENUM_TEMPLATE = os.path.join(HERE, 'microcode', 'MicrocodeTypes.h.templ')
...
class MicroAssembler(object):
    ...
    def _build_microcode(self, files):
        ...
        for f in files:
            ...
            for d in model.lines:
                line, _ = model._tx_parser.pos_to_linecol(d._tx_position) # mm._tx_parser.pos_to_linecol(d._tx_position)
                line -= line_offset + 1
                if token_type(d) == 'LabelAnchor':
                    ...
                    if d.Filename == f:
                        line, _ = model._tx_parser.pos_to_linecol(d._tx_position)
    ...
    microcode_fields = {
        ...
        'jmp_if_fpu_error': (0, partial(_jump, jump_type=JumpType.FPU_ERROR), False),
