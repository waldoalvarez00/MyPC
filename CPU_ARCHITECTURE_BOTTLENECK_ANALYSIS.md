...
```
Total Lines:      512
Line Size:        8 words (16 bytes)
Total Size:       4,096 bytes (4 KB)
Tag Bits:         6 (19 - 3 - 10)
Index Bits:       10 (covers 1024 locations)
Offset Bits:      3 (8 words per line)
Address Range:    19-bit physical (512 KB addressable)
```

### Tag-Index-Offset Calculation

```
Address[19:1]: [Tag(6) | Index(10) | Offset(3)]
               [19:14] | [13:4]     | [3:1]
```

**Hit Condition:**
```verilog
hit = accessing && ((valid && tags_match) ||
    (busy && filling_current && line_valid[offset]));
```

**Cache States:**
- `IDLE`: Waiting for access request
- `BUSY`: Filling cache line from memory
- `FLUSHING`: Writing dirty line back to memory
- `UPDATING`: Checking hit/miss condition

### Cache Line Fill/Flush Operations

**Fill Operation (on miss with clean line):**
1. Initiate memory read of 8 words
2. Increment address for each word receipt
3. Write each word to BlockRam with line_idx[3:1]
4. Set valid bit for each received word

**Flush Operation (on miss with dirty line):**
1. Read current tag from TagRam
2. Reconstruct address: [tag | index | 3'b0]
3. Write entire line back to memory (8 words)
4. Clear dirty bit after flush complete

**Writeback Policy:** Write-through (updates memory on write hit)

### Cache Control Signals

```verilog
Tag RAM:   Dual-port, width=6 bits, 512 entries
Valid RAM: Dual-port, width=1 bit, 512 entries
Dirty RAM: Dual-port, width=1 bit, 512 entries
Data RAM:  Dual-port BlockRam, width=16 bits, 4096 words
```

### Cache Timing

- **Hit Latency:** 1 cycle (combinational tag match + valid check)
- **Miss with fill:** 8+ cycles (1 per word + arbitration)
- **Dirty line flush:** 8 cycles (write) + fill cycles

**Critical Bottleneck:** Each cache miss causes minimum 8-cycle stall
