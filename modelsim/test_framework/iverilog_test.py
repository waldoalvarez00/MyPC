...
class LegacyBashTest(BaseTest):
    """
    Wrapper for running existing bash test scripts.

    This allows gradual migration while keeping old tests working.
    """

    # Path to bash script (relative to modelsim directory)
    script: str = ""

    def __init__(self, script: str = "", **kwargs):
        # Extract name from script before calling super().__init__
        if script and 'name' not in kwargs:
            name = os.path.basename(script).replace("run_", "").replace("_test.sh", "").replace("_sim.sh", "").replace(".sh", "")
            kwargs['name'] = name
    ...
    def compile(self) -> bool:
        """Legacy tests handle their own compilation."""
        return True
    ...
    def run_simulation(self) -> tuple:
        ...
        Returns:
            Tuple of (exit_code, stdout, stderr)
        """
        script_path = os.path.join(get_modelsim_dir(), self.script)
