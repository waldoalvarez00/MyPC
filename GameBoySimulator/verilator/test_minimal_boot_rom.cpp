...
int main() {
    ...
    printf("=== Minimal Boot ROM Test (Interrupt Storm Fix Verification) ===\n\n");

    // Create minimal boot ROM (matching sim_main_gui.cpp logic)
    ...
    // 2. Set up palette
    minimal_boot[pc++] = 0x3E;  // LD A, $1B
    minimal_boot[pc++] = 0x1B;
    minimal_boot[pc++] = 0xE0;  // LDH ($FF47), A  ; BGP
    minimal_boot[pc++] = 0x47;

    // 3. Fill VRAM with simple pattern (simplified version)
    minimal_boot[pc++] = 0x21;  // LD HL, $8000
    minimal_boot[pc++] = 0x00;
    minimal_boot[pc++] = 0x80;
    minimal_boot[pc++] = 0x06;  // LD B, $08
    minimal_boot[pc++] = 0x08;
    int vram_loop = pc;
    minimal_boot[pc++] = 0x3E;  // LD A, $FF
    minimal_boot[pc++] = 0xFF;
    minimal_boot[pc++] = 0x22;  // LD (HL+), A
    minimal_boot[pc++] = 0x22;  // LD (HL+), A
    minimal_boot[pc++] = 0x3E;  // LD A, $00
    minimal_boot[pc++] = 0x00;
    minimal_boot[pc++] = 0x22;  // LD (HL+), A
    minimal_boot[pc++] = 0x22;  // LD (HL+), A
    minimal_boot[pc++] = 0x05;  // DEC B
    minimal_boot[pc++] = 0x20;  // JR NZ, vram_loop
    minimal_boot[pc++] = (uint8_t)(vram_loop - (pc + 1));

    // 4. Turn on LCD
    minimal_boot[pc++] = 0x3E;  // LD A, $91
    minimal_boot[pc++] = 0x91;
    minimal_boot[pc++] = 0xE0;  // LDH ($FF40), A  ; LCDC = $91
    minimal_boot[pc++] = 0x40;

    // 5. Disable boot ROM and jump to cart
    ...
    printf("Minimal boot ROM created: %d bytes\n", pc);
    printf("  DI instruction at start: YES\n");
    printf("  LCD initialization: YES\n");
    printf("  Boot ROM disable: YES\n\n");
    ...
    for (int cycle = 0; cycle < 20000; cycle++) {
        ...
        if (boot_enabled && !dut->dbg_boot_rom_enabled) {
            ...
            printf("  PC: 0x%04X\n", pc);
            printf("  SP: 0x%04X\n", sp);
            printf("  Boot ROM completed successfully\n\n");
        }
    ...
    } else if (boot_complete_cycle >= 0) {
        printf("✅ PASS: Boot ROM completed without interrupt storm\n");
        printf("   Boot completed at cycle %d\n", boot_complete_cycle);
        printf("   No PC stuck at 0x0038-0x003F\n");
    } else {
        printf("⚠️  WARN: Boot ROM still running after 20000 cycles\n");
        printf("   (This is OK - boot ROM may be displaying logo)\n");
