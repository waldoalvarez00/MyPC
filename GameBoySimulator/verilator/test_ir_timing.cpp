...
int main() {
    ...
    printf("=== IR Timing Debug Test ===\n");
    printf("Monitoring PC $0152-$0157 with IR, MCycle, and T-state details\n\n");
    ...
    for (int cycle = 0; cycle < 100000; cycle++) {
        ...
        // Monitor the JR instruction region
        if (boot_completed && pc >= 0x152 && pc <= 0x157) {
            // Print on PC change OR MCycle change (to see M1, M2, M3)
            if (pc != last_pc || mcycle != last_mcycle) {
                printf("Cycle %6d: PC=$%04X IR=$%02X MCycle=%d TState=%d\n",
                       cycle, pc, ir, mcycle, tstate);
                last_pc = pc;
                last_mcycle = mcycle;
            }

            // If we're at $0154, JR failed (should skip)
            if (pc == 0x154) {
                printf("\n❌ FAILURE: JR did not jump - reached $0154 (should skip)\n");
                printf("IR was $%02X when we reached the skip location\n", ir);
                break;
            }

            // If we're at $0156, JR succeeded
            if (pc == 0x156) {
                printf("\n✅ SUCCESS: JR jumped correctly to $0156!\n");
                break;
            }
        }

        if (cycle == 99999) {
            printf("\n⏱️ TIMEOUT: Did not reach test region after 100k cycles\n");
        }
    }
