...
int main() {
    ...
    rom[0x154] = 0x3F;  // CCF (complement/clear carry)
    ...
    for (int cycle = 0; cycle < 300000; cycle++) {
        ...
        if (boot_completed && pc >= 0x150 && pc <= 0x159 && pc != last_pc) {
            ...
            if (pc == 0x154) {
                printf("    → JR C worked (jumped from $0151 to $0154)\n");
            }
            if (pc == 0x158) {
                printf("    → ✅ JR NC worked (jumped from $0155 to $0158)\n");
                printf("\nAll tests PASSED!\n");
                break;
            }
            if (pc == 0x157) {
                printf("    → ❌ JR NC failed (didn't jump, fell through to $0157)\n");
                break;
            }
