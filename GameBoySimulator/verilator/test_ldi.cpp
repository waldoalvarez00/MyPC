// Regression test for LD A,(HL+) instruction (opcode 0x2A)
// Tests that HL increments correctly and A receives the memory value
...
int main(int argc, char** argv) {
    ...
    printf("=== LD A,(HL+) Regression Test ===\n\n");
    ...
    // Run until we reach the loop at PC=0x0105 with JP executing
    ...
    for (int cycle = 0; cycle < 100000 && !reached_loop; cycle++) {
        ...
        // Check if we're at the JP loop
        if (pc == 0x0105 && ir == 0xC3) {
            reached_loop = true;
        }
    }
    ...
    bool pass = (hl == 0x0152) && (a == 0xBB);

    if (pass) {
        printf("\n*** PASS ***\n");
    } else {
        printf("\n*** FAIL ***\n");
        if (hl != 0x0152) printf("  HL incorrect: got 0x%04X, expected 0x0152\n", hl);
        if (a != 0xBB) printf("  A incorrect: got 0x%02X, expected 0xBB\n", a);
    }
